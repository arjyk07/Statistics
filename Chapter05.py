...
# Chapter 5 정규선형모델
# 파이썬으로 배우는 통계학(update: 2023.01.17)(~5.1 진행중)
# 파이썬으로 배우는 통계학(update: 2023.01.18)(~5.1 완료)
# 파이썬으로 배우는 통계학(update: 2023.01.27)(~5.2 진행중)
...
# → 잔차가 정규분포를 따르면 선상에 위치함. 선상에 위치하면 문제없다는 것.

# Q-Q 플롯 직접 만들기
# (실제 데이터의 분위점) 우선 데이터를 앞이 작은 것이 오게 정렬
resid_sort = resid.sort_values()
resid_sort.head()

# (이론 상의 분위점) 이번 데이터는 샘플사이즈 = 30
...
# (이론 상의 분위점)은 정규분포의 퍼센트포인트를 사용하면(이론 상의 누적확률에 대한 분위수값) 계산할 수 있다.
...
stats.norm.ppf(0.03225806)      # -1.8485963510064942
stats.norm.ppf(0.06451613)      # -1.5179291519174756
stats.norm.ppf(0.96774194)      # 1.8485963510064949

"""
    정규분포의 분위수(quartile) 구하기             
    # 참고 사이트 : https://m.blog.naver.com/pmw9440/221805677577
    ex) 어느 고등학교생의 전교생의 상위 5%의 수학점수는 얼마인지를 구하라는 문제를 가끔보게 됩니다. 
        이것을 수학적인 용어로 바꾸면 0.95 분위수를 구하라는 말이 됩니다. 
        만약 특정분포가 정규분포이면 Python에서는 scipy 패키지의 norm.ppf() 함수를 이용해서 구할 수 있습니다.
...
"""
    ...
    - 왜도 : 히스토그램의 좌우비대칭 방향과 그 정도를 측정하는 지표. 
        Skew = E((x - μ)^3 / σ^3)       
            * E(): 기대값을 받는 함수, x: 확률변수(이 경우에는 잔차), μ: x의 평균값, μ: x의 표본표준편차  
        왜도>0 : 오른쪽 자락이 길어짐 
    - 첨도 : 히스토그램 중심부의 뾰족함을 측정하는 지표. 값이 클수록 뾰족해짐.
...
"""
    ex) 종속변수 : 매상
        독립변수 : 날씨(흐림, 비, 맑음)
        → 일원분석 분산분석으로 날씨에 따라 매상이 변화한다고 말할 수 있는가 검정
"""


# p273  5.2.2   분산분석이 필요한 시기
"""
    분산분석 : 평균값의 차이를 검정하는 방법
    분산분석을 사용해야할 때
    - 3개 이상의 수준 간의 평균값에 차이가 있는지 검정할 때
    ex) 날씨가 (맑음, 비, 흐림) 3가지 경우 맥주 매상이 유의미하게 차이가 나는지 어떤지 판단할 경우
    ex) t검정은 (약먹기 전, 약먹은 후) 2개 수준 간의 평균값의 차이 검정
    
    분산분석은 모집단이 정규분포를 따르는 데이터에 대해서만 적용할 수 있음.
    또한 수준 사이의 분산값이 다르지 않다는 조건도 충족해야함.
"""
...
"""
    ...
    ex) 날씨가 (맑음, 비, 흐림) 3가지 수준으로 매상이 달라지는지 검정할 때,
        맑음 : 비,
        비 : 흐림,
        맑음 : 흐림
        3가지 조합으로 t검정 실시하면, 검정 다중성 문제 발생
    
    ex) 유의수준 0.05일 때, 제1종 오류 확률 5%
        if) 검정 2번 연속 실시한다고 할 때, 각 검정의 유의수준이 0.05이면,
            (어느 한쪽의 검정에 대해서라도 귀무가설을 기각할 수 있다면 대립가설을 채택한다는 규칙)
            그러면 제1종 오류 저지를 확률 = 1-(0.95*0.95) = 0.0975가 되어 거의 10%가 됨.
            따라서 검정을 반복하면 귀무가설이 기각되기 쉬워지고 제1종 오류 저지를 확률 높아짐.
            
    → 분산분석을 하게 되면 맑음 or 비와 같은 개별 카테고리를 보는 것이 아니라
      날씨에 따라 매상이 다른지 어떤지 한 번의 검정으로 판단할 수 있게 됨.
"""


# p274  5.2.4   분산분석의 직감적 사고방식 : F비
"""
    귀무가설 : 수준 간의 평균값에 차이가 없다.
    대립가설 : 수준 간의 평균값에 차이가 있다.
    ※ 수준 : 날씨, 물고기의 종류 등과 같은 카테고리형 변수
    ...
    '효과' : 날씨에 따른 매상의 변동(다른 날씨 간의)
    '오차' : 날씨라는 변수를 이용해서 설명할 수 없는 맥주 매상의 변동(같은 날씨 안에서)
    
    영향의 크기는 분산을 이용해서 정량화한다.
    날씨에 따른 매상 변동은 '날씨가 변함에 따른 데이터의 흩어짐 크기'로 표현된다.
    오차 영향의 크기도 마찬가지로 잔차의 분산을 계산함으로써 구할 수 있다.
    
    분산분석 : 분산의 비율을 취한 통계량으로 검정을 시행하기 때문에 분산분석이라고 불린다.
             Analysis of Variance, ANOVA
             
    F비가 크면 '오차'에 비해 '효과'의 영향이 클 것이라고 판단할 수 있다.
    F분포 : 모집단분포가 등분산 정규분포일 때의 F비의 표본분포            
     F분포의 누적분포함수를 사용해서 p값을 계산하고, 
     p값이 0.05이하가 되면 귀무가설을 기각한다는 흐름(t검정과 마찬가지)
"""


# p275  5.2.5   유의미한 차이가 있을 때와 없을 때의 바이올린플롯


# p276  5.2.6   분산분석의 직감적 사고방식: 오차 및 효과의 분리
"""
    '효과'의 크기 : 바이올린 간의 거리
    '오차'의 크기 : 각 바이올린의 폭
   ...
   상 |       ***      │      효과의 크기  │     *****      │
      ...
      바이올린이 서로 떨어져 있다는 것은 날씨에 따라 매상이 크게 변한다는 것을 나타낸다.
      때문에 바이올린 간의 거리가 날씨 효과의 크기가 된다.
      
      같은 날씨라고 해도 매상은 일정하지 않고 흩어진다.
      그래서 날씨로 설명할 수 없는 차이의 크기를 오차의 크기로 표현한 것이다.                                    
"""


# p277  5.2.7   군간변동과 군내변동
"""
    군간변동 : 바이올린 간의 거리, 즉, '효과'의 크기
    군내변동 : 각 바이올린의 폭, 즉, '오차'의 크기
    
    분산분석에서는 데이터의 분산을 군간변동과 군내변동 2개로 나눈 뒤,
    그 비율을 취한 것을 통계량으로 사용하여 검정을 시행한다.
"""
...
# p279  5.2.10  분산분석(1): 군간 제곱과 군내 제곱 계산
# 날씨에 의한 영향
# 흐린 날, 비오는 날, 맑은 날 각 2일씩 있으므로 날씨별 매상은 아래와 같이 될 것으로 기대함.
effect = [7,7,3,3,11,11]

# 군간제곱합
# 이 effect의 흩어진 정도를 구함으로써 군간변동을 구할 수 있다.
# 군간변동의 분자에 해당하는 군간 편차제곱합을 계산한다.
mu_effect = sp.mean(effect)     # 7
...
# p281  5.2.11  분산분석(2): 군간 분산과 군내 분산 계산
"""
    표본분산 : 편차제곱합 / 샘플사이즈
    불편분산 : 편차제곱합 / (샘플사이즈 - 1)
    
    군간변동의 자유도 : 수준의 종류 수에 따라 좌우된다.
     ex) 날씨의 3가지 수준(흐림, 비, 맑음)에서 1을 빼면,
         군간변동의 자유도 = 3 - 1 = 2
         
    군내변동의 자유도 : 샘플사이즈와 수준의 종류 수에 따라 좌우된다.
     ex) 샘플사이즈 = 6, 수준(흐림, 비, 맑음)이 3가지
        따라서, 군내변동의 자유도 = 6 - 3 = 3
"""
...
variance_model      # 32 = 64 / 2

# 군내 분산
# 군내 평균제곱(분산)
variance_resid = squares_resid / df_resid
variance_resid      # 2 = 6 / 3
