...
# 파이썬으로 배우는 통계학(update: 2022.12.21)(~3.10 진행중)
# 파이썬으로 배우는 통계학(update: 2022.12.27)(~3.10 진행중)
...
click_data = pd.read_csv("source/sample/3-10-1-click_data.csv")
...
stats.ttest_rel(after, before)  # Ttest_relResult(statistic=2.901693483620596, pvalue=0.044043109730074276)
...
"""
    독립표본 t검정 : '평균값의 차이'
    대응표본 t검정 : '데이터의 차이'를 보고 1집단 t검정
"""


# p206  3.9.6   독립표본 t검정(실습)
# 평균값
...
# 독립표본 t검정
stats.ttest_ind(after, before, equal_var = False)   # Ttest_indResult(statistic=3.1557282344421034, pvalue=0.013484775682079892)
stats.ttest_ind(before, after, equal_var = False)   # Ttest_indResult(statistic=-3.1557282344421034, pvalue=0.013484775682079892)
# → p값이 0.05보다 작으므로 유의미한 차이가 있다
# → 그러나 대응표본 t검정 결과와 차이가 있음(0.03 정도)
# → 같은 데이터로 같은 목적의 검정을 해도 검정 방법이 달라지면 p값도 달라짐


# p207  3.9.7   독립표본 t검정(등분산)
"""
    등분산이 무엇인지 알아볼 필요 없이 항상 분산이 다르다고 가정한 t검정을 사용해도 상관없음.
    즉, 언제나 Welch 검정 사용해도 됨.
    stats.ttest_ind함수에 equal_var = False를 지정해서 넘기면,
    이는 분산이 다르다고 가정한 t검정을 시행하겠다고 지정하는 것.
    이렇게 하면 Welch검정을 채택하는 것이 됨.
"""
...
"""
    ...
    ※ 데이터 분석은 현실을 알기 위해 하는 것(O) 원하는 결과를 얻기 위해 하는 것(X)
      → 분석가라면 데이터만큼은 성실하게 대하자!
"""



# p208  3.10    분할표 검정(카이제곱 검정)
# p209  3.10.1  분할표를 사용할 때의 장점
# ex) 파란 버튼 vs 빨간 버튼 : 클릭수 + 클릭하지 않은 수, 샘플사이즈


# p210  3.10.2  이 절에서 다룰 예제
# ex) 버튼을 버튼 색에 따라 더 많이 클릭하는지 보여주는 것이 목표


# p210  3.10.3  기대도수 구하기
# 카이제곱 검정을 할 때는 기대도수가 전부 5이상 이어야 한다.


# p210  3.10.4  기대도수와의 차이 구하기
# 카이제곱 통계량 구하기
...
# 카이제곱 검정 correction = False 로 보정하지 않음(불필요한 보정 방지)
sp.stats.chi2_contingency(cross, correction = False)
# 카이제곱 : 6.666666666666666,
...
# p214  3.11    검정 결과 해석
# p214  3.11.1  p값이 0.05 이하일 때의 결과 작성법
# OO은 XX와 유의미한 차이가 있다.


# p214  3.11.2  p값이 0.05보다 클 때의 결과 작성법
# OO은 XX와 유의미한 차이가 있다고 말할 수 없다.


# p214  3.11.3  가설검정을 할 대 자주 하는 실수
""" 
    다음 같은 생각은 틀린 생각!
    1. p값이 작으면 차이가 크다고 말할 수 있다.
    → 샘플사이즈, 표준편차(분산)에 t값이 영향을 받고, 계측 단위가 다를 수도 있음.
    → 따라서 원래 데이터의 평균값을 확인하거나 상자그림이나 
      바이올린플롯을 활용하는 등의 방법을 이용해서 실제와 해석의 차이를 줄일 수 있음
      
    2. p값이 0.05보다 크므로 차이가 없다고 말할 수 있다.
    → 귀무가설이 틀릴 확률은 p값이라는 지표로 컨트롤 가능하지만 귀무가설이 옳을 확률은 컨트롤 할 수 없기 때문
      즉, p값이 0.05보다 크더라도 차이가 없다고 말할 수는 없다.
      
    3. 1-p값은 대립가설이 올바를 확률이다.    
"""


# p215  3.11.4  제1종 오류와 제2종 오류
"""
    제1종 오류 : 귀무가설이 올바르지만 실수하여 귀무가설을 기각하는 경우(무죄인데 유죄결론)(p값)
    → p값이 충분히 작으면 제1종 오류를 범할 확률이 낮아져서 귀무가설을 기각해도 괜찮다고 판단하게 됨
    제2종 오류 : 귀무가설이 틀렸는데 귀무가설을 채택하는 경우(유죄인데 무죄결론)
"""


# p215  3.11.5  검정의 비대칭성
"""
    검정의 비대칭성 : 제1종 오류에 대해서는 그 확률을 컨트롤할 수 있지만, 
                    제2종 오류에 대해서는 컨트롤할 수 없어서 발생하는 것
                    즉, 제2종 오류가 일어날 확률, 즉 귀무가설이 틀렸는데 실수하여
                    귀무가설을 채택할 확률은 통계적가설검정으로 컨트롤 할 수 없다.
"""


# p215  3.11.6  유의수준은 검정을 하기 전에 정해둔다


# p216  3.11.7  모델 선택


# p216  3.11.8  통계적가설검정이 필요한가
# 언젠가는 필요없게 될 지 모른다. 하지만 그게 오늘은 아니다(Maybe so, sir. But not today.)


# p216  3.11.9  가정이 올바른가
# 데이터 분석은 데이터를 다듬는 작업이라고 생각할 수 있지만 실제로는 매우 다르다.
# 표본이 가지는 가정과의 차이를 어떻게 줄일지 항상 생각해야 한다.
